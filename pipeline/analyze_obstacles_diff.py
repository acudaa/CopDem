...
FIELDS = {
    ...
    "error_single_lidar_m": {
        ...
        "unit": "m",
        "kind": "diverging",
        "description": "LiDAR DEM (single-cell, bilinear) minus obstacle base elevation, both on EGM2008. Positive = LiDAR reads above the true base. Vertical datum: GHA height (EPSG:5778, 'Adria Triest') - corrected from an earlier instructed EVRF2000 Austria assumption after being questioned; matched to BEV's product-line documentation and confirmed numerically, but not to this exact file's own embedded metadata. See docs/lidar_comparison.md.",
    },
...
<div class="wrap">
  <header>
    ...
    <p>{n_total:,} obstacles (simple points, v1 scope) compared against two DEM sources &#8212;
       ...
       LiDAR-specific caveats (including the GHA height vertical-datum correction, matched to
       BEV's product documentation but not to this exact file's own embedded metadata) in
