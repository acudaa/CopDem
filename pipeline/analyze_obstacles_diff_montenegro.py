...
different rather than differently-parameterized: (1) Montenegro has only ONE
DEM source (no LiDAR ground truth - see docs/montenegro.md), so there's no
per-source grouping to render; (2) Montenegro's obstacle set has a
fundamentally different scope (ENR 5.4's >=100m-AGL-only filter vs Austria's
full national database - see obstacles_montenegro.py) that this report must
surface prominently, not bury in a footnote shared with Austria's very
different caveats.

Reuses the exact same stats/histogram/by-type functions (copy, not import,
for the same "don't destabilize a working, verified Austria report" reason
export_word_reports.py's Montenegro functions live alongside the Austria
ones rather than replacing them) and the same dataviz-skill chart design
(diverging blue/red, horizontal by-type bars, hover tooltips, zero always
labeled on histograms).

See docs/output_fields.md / docs/montenegro.md for field meanings.
"""
import csv
import html
...
DIFF_CSV = ROOT_DIR / "data" / "obstacles" / "montenegro" / "obstacles_dem_diff.csv"
OUTPUT_HTML = ROOT_DIR / "reports" / "obstacles_dem_diff_analysis_montenegro.html"
...
def histogram(values, n_bins=16):
    # Fewer bins than Austria's 24 - Montenegro's n=51 would leave most of a
    # 24-bin histogram at 0-1 counts per bin, a noisier picture than the
    # data supports.
...
# ---------------------------------------------------------------------------
# SVG rendering (same approach as analyze_obstacles_diff.py, incl. the
# zero-tick-always-labeled fix)
# ---------------------------------------------------------------------------
...
def svg_bar_by_type(type_stats, kind, chart_id, width=640):
    ...
    zero_line = f'<line x1="{zero_x:.2f}" y1="{pad_t}" x2="{zero_x:.2f}" y2="{height - pad_t}" class="zero-line"/>'
...
def build_report():
    ...
    for field_key, meta in FIELDS.items():
        ...
        <section class="card">
          ...
          {render_table(type_stats[field_key])}
...
<title>Obstacle vs. DEM — Analysis (Montenegro)</title>
<style>
  :root {{
    color-scheme: light;
  ...
  * {{ box-sizing: border-box; }}
  ...
  header p {{ color: var(--text-secondary); margin: 0 0 8px; font-size: 14px; }}
  .scope-note {{
    background: var(--surface); border: 1px solid var(--border); border-left: 3px solid var(--series-pos);
    border-radius: 6px; padding: 12px 16px; margin: 0 0 28px; font-size: 13px; color: var(--text-secondary);
  }}
  .card {{
...
<div class="wrap">
  <header>
    <h1>DEM vs. obstacle base &#8212; analysis (Montenegro)</h1>
    <p>{n_total:,} obstacles compared against Copernicus DEM GLO-30 &#8226;
       generated by <code>pipeline/analyze_obstacles_diff_montenegro.py</code> from <code>obstacles_dem_diff.csv</code>.
       Field definitions: see <a href="../docs/output_fields.md">docs/output_fields.md</a>;
       obstacle-source scope and vertical-datum notes in
       <a href="../docs/montenegro.md">docs/montenegro.md</a>.</p>
  </header>

  <div class="scope-note">
    <strong>Scope, read before comparing to Austria:</strong> Montenegro's obstacle list is
    sourced from AIP ENR 5.4 ("Air Navigation Obstacles"), which by its own stated scope covers
    only <strong>permanent obstacles &#8805;328 FT (100 M) AGL</strong> &#8212; not a full national
    obstacle inventory like Austria's dataset. That's why n={n_total} here vs. ~1,932 for Austria:
    a real difference in what's measured, not a smaller country having proportionally fewer
    obstacles recorded. All {n_total} points are either antennas or wind turbines &#8212; no
    towers, buildings, chimneys, cranes, etc. appear in this data set, unlike Austria's broader mix.
    No local high-resolution ground-truth DEM (LiDAR) is available for Montenegro, so &#8212;
    unlike the Austria reports &#8212; this report compares against Copernicus DEM GLO-30 only.
  </div>
  ...
  <footer>
    ...
    See <a href="../docs/montenegro.md">docs/montenegro.md</a> for sampling method, the EGM96
    vertical-datum decision (and why Trieste height/EPSG:5195 — Montenegro's true national datum —
    couldn't be used directly), and known limitations.
  </footer>
</div>
<div id="tooltip"></div>
<script>
